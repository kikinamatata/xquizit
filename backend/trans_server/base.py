...
class ServeClientBase(object):
    ...
    chunking_mode: str
    """Audio chunking mode - 'vad' for VAD-based or 'time_based' for fixed-interval chunking."""
    chunk_interval: float
    """Interval in seconds for time-based chunking mode."""
    ...
    def speech_to_text(self):
        ...
        while True:
            ...
            # Handle different chunking modes
            if self.chunking_mode == "time_based":
                # Time-based: trigger transcription every chunk_interval seconds
                ...
                # VAD mode: current behavior - wait for minimum 1 second of audio
                if duration < 1.0:
    ...
    def flush_audio_buffer(self):
        ...
        This method is called when END_OF_AUDIO signal is received to ensure
        all buffered audio is transcribed and any incomplete segments are marked
        as complete before the connection closes.

        This is a template method that should be overridden by subclasses to
        implement backend-specific flush logic.
        """
        logging.info(f"Client {self.client_uid}: Flushing audio buffer...")

        # Default implementation: mark current output as complete if exists
        if self.current_out and self.current_out.strip():
            try:
                with self.lock:
                    # Calculate duration of remaining audio
                    if self.frames_np is not None:
                        duration = self.frames_np.shape[0] / self.RATE
                        end_time = self.timestamp_offset + duration
                    else:
                        end_time = self.timestamp_offset

                    # Mark current output as completed segment
                    completed_segment = self.format_segment(
                        self.timestamp_offset,
                        end_time,
                        self.current_out,
                    ...
                    if self.translation_queue:
                        try:
                            ...
                            logging.warning("Translation queue is full during flush")

                    logging.info(f"Client {self.client_uid}: Finalized pending segment: '{self.current_out[:50]}...'")

                # Send final segments to client
                segments = self.prepare_segments()
                if segments:
                    self.send_transcription_to_client(segments)
                    logging.info(f"Client {self.client_uid}: Sent {len(segments)} final segments")
                ...
                logging.error(f"Client {self.client_uid}: Error during flush: {e}")
