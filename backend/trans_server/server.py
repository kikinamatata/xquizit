...
class TranscriptionServer:
    ...
    def __init__(self):
        ...
        self.preloaded_model_name = None

    def preload_model_async(self, model_name="distil-large-v3", cache_path="~/.cache/whisper-live/"):
        """
        Preload the model in a background thread if single_model mode is enabled.

        Args:
            model_name (str): Model to preload. Default is "distil-large-v3".
            cache_path (str): Cache directory for models.
        """
        import threading
        import time
        ...
        def preload_worker():
            ...
            try:
                ...
                logging.info(f"Preloading model '{model_name}' in background...")

                # Import the backend class
                from whisper_live.backend.faster_whisper_backend import ServeClientFasterWhisper

                # Call the class method to preload
                success = ServeClientFasterWhisper.preload_model(
                    model=model_name,
                ...
                if success:
                    ...
                    self.preloaded_model_name = model_name
                    logging.info(f"Model preloading complete: {model_name} (took {elapsed_time:.1f}s)")
                else:
                    logging.error(f"Model preloading failed for: {model_name}")

            except Exception as e:
                logging.error(f"Exception during model preloading: {e}")
            finally:
                self.model_preloading = False

        # Start preloading in a daemon background thread
    ...
    def initialize_client(
        ...
        if self.backend.is_tensorrt():
            try:
                ...
                client = ServeClientTensorRT(
                    ...
                    chunking_mode=options.get("chunking_mode", "vad"),
                    chunk_interval=options.get("chunk_interval", 2.0),
                )
        ...
        try:
            if self.backend.is_faster_whisper():
                ...
                # Check if model is still preloading
                if self.single_model and self.model_preloading and not self.model_preload_complete:
                    logging.info("Model is still preloading, client will wait...")

                # If preloaded model exists, log it
                if self.single_model and self.model_preload_complete and self.preloaded_model_name:
                    logging.info(f"Using preloaded model: {self.preloaded_model_name}")
    ...
    def process_audio_frames(self, websocket):
        ...
        if frame_np is False:
            # END_OF_AUDIO signal received - flush remaining buffer
            logging.info(f"END_OF_AUDIO received for client {client.client_uid}, flushing buffer...")
            try:
                client.flush_audio_buffer()
                # Give time for final segments to be processed and sent
                time.sleep(0.3)  # Reduced for speed optimization
                logging.info(f"Buffer flush completed for client {client.client_uid}")
            except Exception as e:
                logging.error(f"Error during buffer flush for client {client.client_uid}: {e}")
    ...
    def run(self,
        ...
        if single_model:
            if faster_whisper_custom_model_path or whisper_tensorrt_path:
                ...
                # Preload custom model asynchronously
                model_to_preload = faster_whisper_custom_model_path if faster_whisper_custom_model_path else None
                if model_to_preload and BackendType(backend).is_faster_whisper():
                    self.preload_model_async(model_name=model_to_preload, cache_path=cache_path)
            else:
                # Enable single_model mode with default distil-large-v3 model
                logging.info("Enabling single model mode with distil-large-v3 model.")
                self.single_model = True
                # Preload default model asynchronously (only for faster_whisper backend)
                if BackendType(backend).is_faster_whisper():
                    self.preload_model_async(model_name="distil-large-v3", cache_path=cache_path)
